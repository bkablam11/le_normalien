...
charClassifications = np.loadtxt("classifications.txt", np.float64) 
flatCharImages = np.loadtxt("flatchar Images.txt", np.float64) 
    ...
    if len(approx) == 4: 
        if len(str(cv2.contourArea(approx))) < 6:
            ...
            for myc in cntr:
                ...
                finalResized = np.float64(finalResized)

                retVal, results, resp, dist = kNearest.findNearest(finalResized, k=1)
                
                tmpString = str(chr(int(results[0][0])))
